...
for i=1:length(spls)
    ...
    for j=1:lns
        for k=j+1:lns
            if A(ns(j),ns(k)) == 1
                tricount = tricount+1;
                skeds(end+1,1:3) = [i,ns(j), 0.1*(lns+degrees(ns(j))) ];                                                        %#ok<AGROW>
                skeds(end,4) = euclidean_distance(spls(i,:), spls(ns(j),:) ); 
                skeds(end+1,1:3) = [ns(j),ns(k), 0.1*(degrees(ns(j)) +degrees(ns(k)) )];                                                %#ok<AGROW>
                skeds(end,4) = euclidean_distance( spls(ns(j),:), spls(ns(k),:) );
                skeds(end+1,1:3) = [i,ns(k), 0.1*(lns+degrees(ns(k))) ];                                                     %#ok<AGROW>
...
while true
    ...
    for j = 1:size(tmpEdges, 1)
        ...
        if ~bin % add edge to skeds
            ...
            skeds(end,3) = 0.1*(degrees(tedge(1))+degrees(tedge(2)));
    ...
    for j = 1:length(rows)
        ...
        skeds(rows(j),3) = 0.1*(degrees(edge(2))+degrees(k));
